...
const WebcamCapture = () => {
  ...
  const [imageWasSaved, setImageWasSaved] = useState(false)
  ...
  const controlImageWasSavedMessageTime = () =>{

    setTimeout(() => {
      setImageWasSaved(false);
  }, 5000);
  }
  ...
  const handleSendImage = async () => {
    try {
      ...
      if(selectedFolder === ''){
        ...
        setImageWasSaved(true);
        controlImageWasSavedMessageTime();
        setShowImage(true)
    }
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div>
        ...
        {imageWasSaved && <p id="image-was-saved">The image was correctly saved</p>}
      </div>
      <datalist id="options">
